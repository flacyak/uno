// Every file uno reads is opened through a FileHandler.
//
...
/** Where each way of reading a file is allowed: inside a handler, and nowhere else. */
const ALLOWED: Array<[RegExp, string[]]> = [
  // The local handler's descriptor, and the store's atomic write.
  [/\bfrom "node:fs(\/promises)?"/, ["store/node.ts"]],
  // The S3 handler's requests.
  [/\bfetch\b/, ["store/s3.ts"]],
  // The blob handler.
  [/\.slice\([^)]*\)\.arrayBuffer\(/, ["store/index.ts"]],
];

test("nothing outside a handler opens a file", () => {
  const found: string[] = [];
